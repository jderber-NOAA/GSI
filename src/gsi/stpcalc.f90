...
!$$$ end documentation block
  ...
  logical,dimension(n0):: usepenterm
...
!$$$
  ...
  use jfunc, only: iout_iter,nclen,xhatsave,yhatsave,&
       iter,nrclen,factqmin,factqmax,factql,factqi,factqr,factqs,factqg, &
       factg,factp,factv,factw10m,facthowv,factcldch,factl
  use jcmod, only: ljcpdry,ljc4tlevs,ljcdfi,ljclimqc,alphajc,bamp_jcpdry
...
! Declare local variables
  integer(i_kind) i,j,mm1,ii,iis,ibin,ipenloc,itl
...
!    penalties for moisture constraint
     if(usepenterm(4) .or. usepenterm(5))then
        if(.not.ljc4tlevs) then
           ...
           do ibin=1,nobs_bins
              if (nobs_bins == nfldsig) itl=ibin
              call stplimq(dval(ibin),sval(ibin),sges,pbcqmin(1,ibin),pbcqmax(1,ibin),nstep,itl)
           end do
           pbc(:,4)=zero_quad
           pbc(:,5)=zero_quad
           do ibin=1,nobs_bins
...
!     penalties for cldch constraint
      ...
      if (usepenterm(13)) then
         if(.not.ljc4tlevs) then
            ...
            do ibin=1,nobs_bins
               if (nobs_bins == nfldsig) itl=ibin
               call stplimqc(dval(ibin),sval(ibin),sges,pbcql(1,ibin),nstep,itl,'ql')
            end do
            pbc(:,13)=zero_quad
            do ibin=1,nobs_bins
               do j=1,nstep
      ...
      if (usepenterm(14)) then
         if(.not.ljc4tlevs) then
            ...
            do ibin=1,nobs_bins
               if (nobs_bins == nfldsig) itl=ibin
               call stplimqc(dval(ibin),sval(ibin),sges,pbcqi(1,ibin),nstep,itl,'qi')
            end do
            pbc(:,14)=zero_quad
            do ibin=1,nobs_bins
               do j=1,nstep
      ...
      if (usepenterm(15)) then
         if(.not.ljc4tlevs) then
            ...
            do ibin=1,nobs_bins
               if (nobs_bins == nfldsig) itl=ibin
               call stplimqc(dval(ibin),sval(ibin),sges,pbcqr(1,ibin),nstep,itl,'qr')
            end do
            pbc(:,15)=zero_quad
            do ibin=1,nobs_bins
               do j=1,nstep
      ...
      if (usepenterm(16)) then
         if(.not.ljc4tlevs) then
            ...
            do ibin=1,nobs_bins
               if (nobs_bins == nfldsig) itl=ibin
               call stplimqc(dval(ibin),sval(ibin),sges,pbcqs(1,ibin),nstep,itl,'qs')
            end do
            pbc(:,16)=zero_quad
            do ibin=1,nobs_bins
               do j=1,nstep
      ...
      if (usepenterm(17)) then
        if(.not.ljc4tlevs) then
           ...
           do ibin=1,nobs_bins
              if (nobs_bins == nfldsig) itl=ibin
              call stplimqc(dval(ibin),sval(ibin),sges,pbcqg(1,ibin),nstep,itl,'qg')
           end do
           pbc(:,17)=zero_quad
           do ibin=1,nobs_bins
              do j=1,nstep
...
subroutine setupstpcalc

  use constants, only: zero
  use jfunc, only: factqmin,factqmax,factql,factqi,factqr,factqs,factqg, &
       factg,factp,factv,factw10m,facthowv,factcldch,factl
  use jcmod, only: ljcpdry,ljc4tlevs,ljcdfi,ljclimqc,alphajc,bamp_jcpdry
  ...
  usepenterm=.false.
  usepenterm(1)=.true.
  if (ljcdfi .and. nobs_bins>1 .and. alphajc > zero) usepenterm(2)=.true.
  if(ljcpdry .and. bamp_jcpdry > zero)usepenterm(3)=.true.
  if(.not. ltlint)then
     if(factqmin >= zero .or. factqmax >=zero)then
        usepenterm(4)=.true.
        usepenterm(5)=.true.
     end if
     if(getindex(cvars2d,'gust')>0 .and. factg >= zero) usepenterm(6)=.true.
     if(getindex(cvars2d,'vis')>0 .and. factv >= zero) usepenterm(7)=.true.
     if(getindex(cvars2d,'pblh')>0 .and. factp >=zero) usepenterm(8)=.true.
     if(getindex(cvars2d,'wspd10m')>0 .and. factw10m >=zero) usepenterm(9)=.true.
     if(getindex(cvars2d,'howv')>0 .and. facthowv >=zero) usepenterm(10)=.true.
     if(getindex(cvars2d,'lcbas')>0 .and. factl >=zero) usepenterm(11)=.true.
     if(getindex(cvars2d,'cldch')>0 .and. factcldch >=zero) usepenterm(12)=.true.
     if (ljclimqc) then
        if(getindex(cvars3d,'ql')>0 .and. factql >=zero) usepenterm(13)=.true.
        if(getindex(cvars3d,'qi')>0 .and. factqi >=zero) usepenterm(14)=.true.
        if(getindex(cvars3d,'qr')>0 .and. factqr >=zero) usepenterm(15)=.true.
        if(getindex(cvars3d,'qs')>0 .and. factqs >=zero) usepenterm(16)=.true.
        if(getindex(cvars3d,'qg')>0 .and. factqg >=zero) usepenterm(17)=.true.
     end if
  end if
end subroutine setupstpcalc
